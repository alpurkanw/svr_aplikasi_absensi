 ...
 */
class MY_Owner_Controller extends CI_Controller
{
    public function __construct()
    {
        ...
        if (!$this->session->userdata('logged_in') || $this->session->userdata('level') != '3') {
            ...
            $this->session->set_flashdata(
                'pesan',
                '<div class= "alert alert-warning" role="alert">Anda tidak memiliki akses ke halaman ini. Silakan login sebagai Owner.</div>'
            );
