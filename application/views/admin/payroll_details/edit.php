            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        <div>
                            <h1 class="h3 text-gray-800 mb-0">Detail Komponen Gaji</h1><small class="text-muted"><?= html_escape($employee['employee_code'] . ' - ' . $employee['name']) ?> &middot; seluruh nominal gaji diatur melalui komponen</small>
                        </div>
                        <a class="btn btn-secondary" href="<?= site_url('admin/employees/detail-page/' . (int) $employee['id']) ?>"><i class="fas fa-arrow-left"></i> Kembali</a>
                    </div>
                    <form id="detailForm">
                        <div class="row">
                            <?php foreach (array('EARNING' => array('title' => 'Penambah / Pendapatan', 'class' => 'success'), 'DEDUCTION' => array('title' => 'Pengurang / Potongan', 'class' => 'danger')) as $type => $group): ?>
                                <div class="col-lg-6 mb-4">
                                    <div class="card shadow">
                                        ...
                                        <div class="card-body">
                                            <?php $found = false;
                                            foreach ($components as $component): if ($component['component_type'] !== $type) continue;
                                                $found = true; ?>
                                            ...
                                            <?php endforeach;
                                            if (!$found): ?><p class="text-muted mb-0">Belum ada komponen aktif.</p><?php endif; ?>
                                        </div>
                                    </div>
                                </div>
                            <?php endforeach; ?>
    ...
    <script>
        ...
        $('#detailForm').on('submit', function(event) {
            ...
            $.post('<?= site_url('admin/payroll-details/' . (int) $employee['id'] . '/save') ?>', $(this).serialize(), function(response) {
                ...
                Swal.fire('Gagal', 'Detail komponen gagal disimpan.', 'error');
            });
        });
    </script>
</body>
